...
def test_read_only_numbers():
    ...
    with patch('SRC.api.API', fake_api):   #w patch podaje jako str scieżke -
        # gdy używam klasy API w pliku api z katalogu SRC zamienia na  obiekt fake_api==moki
        # patch podmienia obiekt oryginalny a context manager określa zakres używania mocka
        result = get_only_numbers()  #stworzona zmienna result przypisuję wynik kunkcji get_only_numbers z moka fake_api
